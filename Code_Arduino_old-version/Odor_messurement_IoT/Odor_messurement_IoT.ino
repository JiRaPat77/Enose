...
// #define WIFI_STA_NAME "Weaverbase"  
// #define WIFI_STA_PASS "1212312121"  
...
char json_7[300];
StaticJsonDocument<200> jsonDocument_7;
char jsonString_7[200];

char json_8[300];
StaticJsonDocument<200> jsonDocument_8;
char jsonString_8[200];
...
void loop() {
  ...
  if (now - lastMsg > 30000) {
    ...
    mqtt.loop();
    float temp_str;
    temp_str = (sht.getTemperature(), 1);
    Serial.println(temp_str);
    jsonDocument_0["Temperature"] = temp_str;
    serializeJson(jsonDocument_0, jsonString_0);
    mqtt.publish("v1/devices/me/telemetry", jsonString_0);


    int hum_str;
    hum_str = (sht.getHumidity(), 1);
    Serial.println(hum_str);
    jsonDocument_1["Humidity"] = hum_str;
    ...
    float so_str;
    so_str = float(so2);
    jsonDocument_3["SO2"] = so_str;
    serializeJson(jsonDocument_3, jsonString_3);
    mqtt.publish("v1/devices/me/telemetry", jsonString_2);


    float wsp_str;
    wsp_str = float((Count * 8.75) / 100);
    jsonDocument_4["Wind Speed"] = wsp_str;
    serializeJson(jsonDocument_4, jsonString_4);
    mqtt.publish("v1/devices/me/telemetry", jsonString_4);


    int wdr_str;
    wdr_str = int(wind_dir_raw);
    jsonDocument_5["Wind Directions"] = wdr_str;
    serializeJson(jsonDocument_5, jsonString_5);
    mqtt.publish("v1/devices/me/telemetry", jsonString_5);


    float pm25_str;
    pm25_str = float(m.mc_2p5);
    jsonDocument_6["PM2.5"] = pm25_str;
    serializeJson(jsonDocument_6, jsonString_6);
    mqtt.publish("v1/devices/me/telemetry", jsonString_6);


    float pm10_str;
    pm10_str = float(m.nc_10p0);
    jsonDocument_7["PM10"] = pm10_str;
    serializeJson(jsonDocument_7, jsonString_7);
    mqtt.publish("v1/devices/me/telemetry", jsonString_7);


    int batt_str;
    batt_str = int(batt);
    jsonDocument_8["PM10"] = batt_str;
    serializeJson(jsonDocument_8, jsonString_8);
    mqtt.publish("v1/devices/me/telemetry", jsonString_8);
